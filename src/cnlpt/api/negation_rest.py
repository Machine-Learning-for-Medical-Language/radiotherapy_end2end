...
async def process(doc: EntityDocument):
    ...
    dataset = ClassificationDocumentDataset.from_instance_list(
        instances, app.state.tokenizer, label_list=NegationProcessor().get_labels()
    )
    preproc_end = time()

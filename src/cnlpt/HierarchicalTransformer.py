...
class HierarchicalModel(CnlpModelForClassification):
    ...
    def forward(
        ...
        Args:
          input_ids(`torch.LongTensor` of shape `(batch_size, optional): A batch of chunked documents as tokenizer indices. (Default value = None)
          attention_mask(`torch.LongTensor` of shape `(batch_size, optional): Attention masks for the batch. (Default value = None)
          token_type_ids(`torch.LongTensor` of shape `(batch_size, optional): Token type IDs for the batch. (Default value = None)
          position_ids: (`torch.LongTensor` of shape `(batch_size, num_chunks, chunk_len)`, *optional*):
        Position IDs for the batch. (Default value = None)
          head_mask(`torch.LongTensor` of shape `(num_heads, optional): Token encoder head mask. (Default value = None)
          inputs_embeds(`torch.FloatTensor` of shape `(batch_size, optional): A batch of chunked documents as token embeddings. (Default value = None)
          labels(`torch.LongTensor` of shape `(batch_size, optional): Labels for computing the sequence classification/regression loss.
        ...
        If `self.num_labels[task_ind] > 1` a classification loss is computed (Cross-Entropy). (Default value = None)
          output_attentions(`bool`, optional): Whether or not to return the attentions tensors of all attention layers. (Default value = None)
          output_hidden_states: not used. (Default value = None)
          event_tokens: not currently used (only relevant for token classification) (Default value = None)
        ...
        if (
            ...
            logger.warning(
                "Attention mask may not be in the right shape for "

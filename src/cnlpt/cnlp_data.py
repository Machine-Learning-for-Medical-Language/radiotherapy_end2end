...
def cnlp_convert_features_to_hierarchical(
    ...
    Args:
      InputFeatures: features: the old instance
      int: chunk_len: the maximum length of a chunk
      int: num_chunks: the maximum number of chunks in the instance
      int: cls_id: the tokenizer's ID representing the CLS token
      int: sep_id: the tokenizer's ID representing the SEP token
    ...
    empty chunk at the beginning of the instance
      ...
      # cls_token_segment_id:  (Default value = 0)
      # pad_token_segment_id:  (Default value = 0)
      # use_special_token:  (Default value = True)
    ...
    input_ids_, attention_mask_, token_type_ids_, event_tokens_, label_ = astuple(
        features
    )
    ...
    return HierarchicalInputFeatures(
        chunks,
        chunks_attention_mask,
        chunks_token_type_ids,
        chunks_event_tokens,
...
def cnlp_convert_examples_to_features(
    ...
    Args:
      typing: List[transformers.data.processors.utils.InputExample] examples:
    the list of examples to convert
      transformers: tokenization_utils.PreTrainedTokenizer tokenizer: the tokenizer
      typing: Optional[int] max_length: the maximum sequence length
    at which to truncate examples
      str: task: the task name
      typing: Optional[typing.List[str]] label_list: the list of labels
    ...
    the processor with :meth:`transformers.DataProcessor.get_labels`.
      typing: Optional[str] output_mode: the output mode for this task.
    If not provided explicitly, it will be retrieved from
    :data:`cnlpt.cnlp_processors.cnlp_output_modes`.
      bool: inference: whether we're doing training or inference only -- if inference mode the labels associated with examples can't be trusted.
      bool: hierarchical: whether to structure the data for the hierarchical
    model (:class:`cnlpt.HierarchicalTransformer.HierarchicalModel`)
      int: chunk_len: for the hierarchical model, the length of each
    chunk in tokens
      int: num_chunks: for the hierarchical model, the number of chunks
      bool: insert_empty_chunk_at_beginning: for the hierarchical model,
    ...
    of the example instances printed to the log
      ...
      output_mode: Optional[str]:  (Default value = None)
      inference: bool:  (Default value = False)
      hierarchical: bool:  (Default value = False)
      chunk_len: int:  (Default value = -1)
      num_chunks: int:  (Default value = -1)
      insert_empty_chunk_at_beginning: bool:  (Default value = False)
      truncate_examples: bool:  (Default value = False)
...
class ClinicalNlpDataset(Dataset):
    ...
    Args:
      DataTrainingArguments: args: the data training args for this experiment
      transformers: tokenization_utils.PreTrainedTokenizer tokenizer: the tokenizer
      typing: Optional[int] limit_length: if provided, the number of
    examples to include in the dataset
      typing: Union[str, Split] mode: the data split mode of this dataset
    (:obj:`"train"`, :obj:`"dev"`, :obj:`"test"`)
      typing: Optional[str] cache_dir: if provided, the directory to save/load a cache
    of this dataset
      bool: hierarchical: whether to structure the data for the hierarchical
    model (:class:`cnlpt.HierarchicalTransformer.HierarchicalModel`)
                ...
                if self.args.weight_classes and mode == Split.train:
                    if num_subtasks == 1:
                        ...
                        class_counts = np.zeros(
                            (num_subtasks, len(self.label_lists[task_ind]))
                        )
                        for feature in features:
                            labels = feature.label[0]
                        ...
                        self.class_weights[subtask_ind] = (
                            min(class_counts[subtask_ind]) / class_counts[subtask_ind]
                        )
